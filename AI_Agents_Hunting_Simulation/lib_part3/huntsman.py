...
class Huntsman(Thing):
    ...
    def travel(self, direction):
        ...
        # Added check to avoid deer location (this is only for Demo purposes in this case)
        
        not_moving = True
        
        if direction == "travel east":
            if(self.location[1] >= self.world_size_y-1):
                if self.avoid_deer_loc([self.location[0],0]):
                    newLocation = [self.location[0],0]
                    not_moving = False
            else:
                if self.avoid_deer_loc([self.location[0],self.location[1]+1]):
                    newLocation = [self.location[0],self.location[1]+1]
                    not_moving = False
            
        elif direction == "travel north":
            if(self.location[0] == 0):
                if self.avoid_deer_loc([self.world_size_x-1,self.location[1]]):
                    newLocation = [self.world_size_x-1,self.location[1]]
                    not_moving = False
            else:
                if self.avoid_deer_loc([self.location[0]-1,self.location[1]]):
                    newLocation = [self.location[0]-1,self.location[1]]
                    not_moving = False
            
        elif direction == "travel west":
            if(self.location[1] == 0):
                 if self.avoid_deer_loc([self.location[0],self.world_size_y-1]):
            ...
            if(self.location[0] >= self.world_size_x-1):
                if self.avoid_deer_loc([0,self.location[1]]):
        ...
        if not_moving: # last option as fallback (same location for next step --> needed for when Deer doesn't work out - crude solution)                    
            newLocation = self.location # same

        # self.energy -= 3 # here obsolete (Part 3 Concept Demo only)
        self.performance -= 1
        # self.check_alive() # not required
    ...
    def avoid_deer_loc(self, location):
        """ Added in V3 - simple check to avoid Deer Location (basically no cheating!)"""        
        
        if location in settings.deer_init_starting_locations:
            return False
        else:            
            return True
